...
between a covaraiate and the outcome of a regression (or the error in your estimate). In the simplest case, 
say you run the following formula in glm `wages ~ age + age^2`. 
...
```{r}
modmarg::mod_marg2(mod = g, var_interest = "treatment", type = 'levels')
```

Or the effect of a unit change in `treatment`.
```{r}
modmarg::mod_marg2(mod = g, var_interest = "treatment", type = "effects")
```
...
`mod_marg2` will try to create multiple orthogonal vectors from a constant, 
which doesn't work so well.
```{r}
g <- glm(y ~ poly(age, 3, raw = T)*as.factor(treatment) , data = margex)
summary(g)

m <- modmarg::mod_marg2(mod = g, var_interest = "treatment", type = "effects",
...
relationship is.
```{r}
mod_marg2(mod = g, var_interest = "treatment", type = 'levels')
mod_marg2(mod = g, var_interest = "treatment", type = "effects")
```
...
age groups while controlling for distance. `mod_marg2` makes that simple.
...
mod_marg2(mod = g, var_interest = "agegroup", type = 'levels',
...
matrices is beyond the scope of this package. However, they can be used with `mod_marg2`
...
(see `modmarg/data-raw/make_cluster_vcov.R` for one method to do this). 
