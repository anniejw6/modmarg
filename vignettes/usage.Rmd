...
for age. In this first example we'll use bins of age we created earlier.
```{r}
library(modmarg)
data(margex)
...
Note that (at least currently), you have to use `raw = T` when using `poly()`. Otherwise
...
m <- modmarg::mod_marg2(g, var_interest = "treatment", type = "effects",
...
relationship is.
```{r}
mod_marg2(g, var_interest = "treatment")
mod_marg2(g, var_interest = "treatment", type = "effects")
```
...
mod_marg2(g, var_interest = "agegroup", 
