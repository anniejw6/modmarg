...
Note that $P$ is a random variable with mean $\bar{P}$ and variance $\text{Var}(P)$. Thus, the first-order [Taylor Expansion](https://en.wikipedia.org/wiki/Taylor_series) around $\overline{P}$ with respect to $X \beta$ is, generally,

$$P(\overline{X\beta}) + 
\frac{d(P(\overline{X\beta}))}{d(X\beta)}(X\beta - \overline{X\beta})$$

For predicted levels, the Taylor expansion is 

$$\text{link}^{-1}(\overline{X\beta}) + 
\frac{d(\text{link}^{-1}(\overline{X\beta}))}{d(X \beta)}(X\beta - \overline{X\beta})$$

for the predicted effects of categorical variables,
$$d(\text{link}^{-1}/d(x \beta)(\bar(X) \beta)(X_1 - X_2)$$

and for the predicted effects of continuous variables,

$$\frac{d(\text{link}^{-1}(\overline{X\beta}))}{d(X \beta)} + 
\frac{d^2(\text{link}^{-1}(\overline{X\beta}))}{d(X\beta)}(X\beta - \overline{X\beta})$$
